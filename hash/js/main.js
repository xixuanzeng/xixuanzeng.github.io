...
function onDown(){
	...
	if(matrix[partX][partY]==0){
		...
		if(win(partX,partY)){
			statusContent = "诶，我家小左太厉害了";
			gameover();
			addText();
		}else if(isEnd()){
			statusContent = "平局咯~，快亲我一口";
			gameover();
			addText();
		}else{
			statusContent = "老公要想想怎么才能赢你";
			addText();
			computerThink();
		}
	}
...
function computerThink(){
	...
	if(win(x,y)){
		statusContent = "我我我我错了，我不该赢的";
		gameover();
		addText();
	}else if(isEnd()){
		statusContent = "平局啦~~么么大";
		gameover();
		addText();
	}else{
		statusContent = "到我家小左下了~";
		addText();
	}
}
function isEnd(){
